...
export const brandingApi = {
  ...
  delete: (orgId: string) => api<{ success: boolean }>(`/api/branding/${orgId}`, { method: 'DELETE' }),
};
...
export const stationsApi = {
  ...
  listTokens: (id: string) =>
    api<any[]>(`/api/stations/${id}/tokens`),
...
export const profilesApi = {
  ...
  importProfile: (profileId: string, targetOrgSigla: string) =>
    api<any>(`/api/profiles/${profileId}/import`, { method: 'POST', body: { targetOrgSigla } }),
};

// Browser Policies API
export const browserPoliciesApi = {
  list: (orgId: string) => api<any[]>(`/api/browser-policies/${orgId}`),
  ...
  }) => api<any>('/api/browser-policies', { method: 'POST', body: data }),
};

// Desktop Policies API
export const desktopPoliciesApi = {
  get: (orgId: string) => api<any>(`/api/desktop-policies/${orgId}`),
  ...
  upsert: (data: {
    ...
    powerTimeout?: number;
  }) => api<any>('/api/desktop-policies', { method: 'POST', body: data }),
};

// Printer Profiles API
export const printerProfilesApi = {
  get: (orgId: string) => api<any[]>(`/api/printer-profiles/${orgId}`),
  ...
  }) => api<any>('/api/printer-profiles', { method: 'POST', body: data }),
};

// Offline Auth API
export const offlineAuthApi = {
  get: (orgId: string) => api<any>(`/api/offline-auth/${orgId}`),
  ...
  upsert: (data: {
    ...
    offlineDays?: number;
    maxOfflineLogins?: number;
    autoSync?: boolean;
    sssdConfig?: string;
    adDomain?: string;
  }) => api<any>('/api/offline-auth', { method: 'POST', body: data }),
};
