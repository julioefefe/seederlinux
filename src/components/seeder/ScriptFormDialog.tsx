    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          {/* Full variable catalog - click to insert */}
          {catalog.length > 0 && (
            <div className="rounded-md border bg-muted/30 p-3 space-y-3">
              <div className="text-xs font-mono uppercase tracking-wide text-muted-foreground">
                Catálogo de variáveis ({catalog.length}) — clique para inserir
              </div>
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {catalog.map((v: any) => (
                  <button
                    key={v.key}
                    type="button"
                    onClick={() => set("conteudo", form.conteudo + `{{${v.key}}}`)}
                    className="w-full flex items-center gap-3 text-xs p-2 rounded-md bg-background border hover:border-primary/40 hover:bg-primary/5 transition-colors text-left"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <code className="font-mono font-semibold">{v.key}</code>
                        {v.obrigatoria && (
                          <Badge variant="outline" className="text-[9px] h-4 border-destructive/30 text-destructive">obrigatória</Badge>
                        )}
                      </div>
                      {v.descricao && (
                        <p className="text-muted-foreground mt-0.5 truncate">{v.descricao}</p>
                      )}
                    </div>
                    {v.tipo && (
                      <Badge variant="outline" className="text-[9px] font-mono shrink-0">{v.tipo}</Badge>
                    )}
                    {v.default && (
                      <span className="text-muted-foreground font-mono shrink-0">padrão: {v.default}</span>
                    )}
                  </button>
                ))}
              </div>
                    ...
                    <div key={k} className={`flex items-center gap-3 text-xs p-2 rounded-md ${isKnown ? "bg-background border" : "bg-destructive/5 border border-destructive/20"}`}>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {!isKnown && (
                            <Badge variant="destructive" className="text-[9px] h-4">não catalogada</Badge>
                          )}
                          {entry?.obrigatoria && (
                            <Badge variant="outline" className="text-[9px] h-4 border-destructive/30 text-destructive">obrigatória</Badge>
                          )}
                      ...
                      {entry?.default && (
                        <span className="text-muted-foreground font-mono shrink-0">padrão: {entry.default}</span>
                      )}
                    </div>
                  );
                })}
